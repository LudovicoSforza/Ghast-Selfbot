...
if os.name == "nt":
    ...
    os.system("title Ghast")
if os.name == "posix":
    os.system("clear")

print(" ")
print(f"{printSpaces}Loading Ghast...")
print(" ")
...
print(" ")
print(printSpaces+"If it stops here please restart ghost")
print(printSpaces+"Give it a second...")
...
try: 
    ...
    if not os.path.isfile('icon.ico'): open('icon.ico', 'wb').write(requests.get('https://clientleaks.xyz/favicon.ico', allow_redirects=True).content);  
    if not os.path.isfile('sounds/connected.mp3'): open('sounds/connected.mp3', 'wb').write(requests.get('https://clientleaks.xyz/assets/sounds/connected.mp3', allow_redirects=True).content);  
    if not os.path.isfile('sounds/error.mp3'): open('sounds/error.mp3', 'wb').write(requests.get('https://clientleaks.xyz/assets/sounds/error.mp3', allow_redirects=True).content);  
    if not os.path.isfile('sounds/notification.mp3'): open('sounds/notification.mp3', 'wb').write(requests.get('https://clientleaks.xyz/assets/sounds/notification.mp3', allow_redirects=True).content);  
    if not os.path.isfile('sounds/success.mp3'): open('sounds/success.mp3', 'wb').write(requests.get('https://clientleaks.xyz/assets/sounds/success.mp3', allow_redirects=True).content);  
    if not os.path.isfile('sounds/giveaway-win.mp3'): open('sounds/giveaway-win.mp3', 'wb').write(requests.get('https://clientleaks.xyz/assets/sounds/giveaway-win.mp3', allow_redirects=True).content);  
    open('data/icon.png', 'wb').write(requests.get('https://clientleaks.xyz/assets/icon.png', allow_redirects=True).content)
...
{
    "enabled": true,
    "client_id": 899029984574840832,
    "details": "Using Ghast selfbot...",
    "state": "",
    "large_image_key": "icon",
    "large_image_text": "ghast.rest"
}
        """)
        f.close()
    ...
    if os.path.isfile("richpresence.json"):
        ...
        if jsonFile["client_id"] == 807369019744059403:
            jsonFile["client_id"] = 899029984574840832
        if jsonFile["details"] == "Using Ghast selfbot...":
            jsonFile["details"] = "Using Ghast..."
        if "small_image_key" not in jsonFile:
            jsonFile["small_image_key"] = "small"
        if "small_image_text" not in jsonFile:
            jsonFile["small_image_text"] = "best free sb"
        json.dump(jsonFile, open("richpresence.json", "w"), sort_keys=False, indent=4)

    if not os.path.isfile('themes/Ghast.json'):
        f = open('themes/Ghast.json', "w")
        f.write("""
{
    "embedtitle": "Ghast",
    "embedcolour": "#3B79FF",
    "consolecolour": "#3B79FF",
    "embedfooter": "ghast.rest",
    "embedfooterimage": "https://clientleaks.xyz/assets/icon.png",
    "globalemoji": ":blue_heart:",
    "embedimage": "https://clientleaks.xyz/assets/icon.png"
}
        """)
        f.close()
...
Never gonna tell a lie and hurt...""")
    ...
    if not os.path.isfile('scripts/example.py'):
        ...
        f.write('''
@Ghast.command(name="example", description="Example custom script.", usage="example")
async def example(Ghast):
    exampleEmbed = discord.Embed(
        ...
        ** **
        Ghast scripts are all created in python using discord.py so you can use any feature from discord.py.
        """,
    ...
    **\_\_deletetimeout\_\_** : Config delete timeout (seconds).
    ...
    await Ghast.send("Hello World!", embed=exampleEmbed)
    ''')
        f.close()
    ...
    for theme in os.listdir("themes"):
        if theme.endswith(".json"):
            ...
            if "embedlargeimage" not in themeJson:
                themeJson["embedlargeimage"] = "https://i.vgy.me/sPmdpT.gif"
    ...
    nsfwTypes = ["Slut", "RealGirls", "Ofleaks_any"]
    now = datetime.now()
    fake = Faker()
    def getCurrentTime():
    ...
    def is_me(m):
        return m.author == Ghast.user
    def restart_bot():
        python = sys.executable
        os.execl(python, python, * sys.argv)
    def close_bot():
        os.system("taskkill /IM Ghast.exe")
    def is_windows():
        return os.name == "nt"
    def is_linux():
    ...
    class Config():
        ...
        def changePrefix(newPrefix):
            ...
            Ghast.command_prefix = newPrefix
            cfg = Config.getConfig()
    ...
    if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
        ...
        status = "online"
    Ghast = commands.Bot(command_prefix=__prefix__, self_bot=True, status=discord.Status.try_value(status))
    Ghast.remove_command('help')
    Ghast.launch_time = datetime.utcnow()
    ...
    if not __custommotd__:
        motd = "Developed by Benny | Discontinued October 2021 | Resumed by qoft, woosh & ghast.rest"
    else:
    ...
    async def on_connect():
        ...
        if is_windows():
            os.system("cls")
            os.system(f"title Ghast [{version}] [{Ghast.user}]")
        ...
        #             bat_file = open(startupPath() + r"\\Ghast.bat", "w")
        ...
        #         if os.path.exists(startupPath() + r"\\Ghast.bat"): os.remove(startupPath() + r"\\Ghast.bat");
        ...
        if consoleMode.lower() == "new2":
            ...
            print("\ \ \__ \  \ \  __ \  \ \  __ \  \ \___  \  \/_/\ \/ ".center(width))
            print(" \ \_____\  \ \_\ \_\  \ \_\ \_\  \/\_____\    \ \_\ ".center(width))
            print("  \/_____/   \/_/\/_/   \/_/\/_/   \/_____/     \/_/ ".center(width))
            print("                                                     ".center(width)) 
            print("")
            print(fg.cWhite + f"{motd}".center(width))
        ...
        if consoleMode.lower() == "new3":
            print("")
            print(fg.consoleColour + "")                    
            print("   ___   __  __     .      _____  _______".center(width))
            print(" .'   \  |   |     /|     (      '   /   ".center(width))
            print(" |       |___|    /  \     `--.      |   ".center(width))
            print(" |    _  |   |   /---'\       |      |   ".center(width))
            print("  `.___| /   / ,'      \ \___.'      /   ".center(width))                    
            print("")
            print(fg.cWhite + f"{motd}".center(width))
            print(fg.consoleColour + '─'*width)
            print("")                    
        ...
        if consoleMode.lower() == "bear":
            ...
            print("")
            print(fg.consoleColour + "")                    
            print("   _     _      _     _      _     _      _     _      _     _   ".center(os.get_terminal_size().columns))
            print("  (c).-.(c)    (c).-.(c)    (c).-.(c)    (c).-.(c)    (c).-.(c)  ".center(os.get_terminal_size().columns))
            print("   / ._. \      / ._. \      / ._. \      / ._. \      / ._. \   ".center(os.get_terminal_size().columns))
            print(" __\( Y )/__  __\( Y )/__  __\( Y )/__  __\( Y )/__  __\( Y )/__ ".center(os.get_terminal_size().columns))
            print("(_.-/'-'\-._)(_.-/'-'\-._)(_.-/'-'\-._)(_.-/'-'\-._)(_.-/'-'\-._)".center(os.get_terminal_size().columns))
            print("   || G ||      || H ||      || A ||      || S ||      || T ||   ".center(os.get_terminal_size().columns))                                      
            print(" _.' `-' '._  _.' `-' '._  _.' `-' '._  _.' `-' '._  _.' `-' '._ ".center(os.get_terminal_size().columns))                                      
        ...
        if consoleMode.lower() == "rise":
            ...
            print("╚═╝  ╚═╝╚═╝╚══════╝╚══════╝    ╚══════╝╚══════╝╚══════╝╚═╝     ╚═════╝  ╚═════╝    ╚═╝   ".center(width))
            print("╭─━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━─╮")
            print(fg.cGrey + f"Connected: {Ghast.user} | Prefix: {Ghast.command_prefix} | Servers: {len(Ghast.guilds)}".center(width))
            print(fg.cBlue + "╰─━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━─╯")
            print("")
        ...
        if consoleMode.lower() == "nighty":
            ...
            print(f"{fg.cWhite}Account:   {Ghast.user} [{len(Ghast.guilds)} servers] [{len(get_friends(__token__))} friends]")
            print(f"{fg.cWhite}Prefix:    {Ghast.command_prefix}")
            ...
            # def print_cmd(command):
            #     print(f"{fg.cGrey}{getCurrentTime()} | {fg.consoleColour}[Command] {fg.cGrey} | {Ghast.command_prefix}{command}")
            # def print_sharecmd(author, command):
        ...
        if "beta" in version.lower():
            print_important("You're currently using a beta build of Ghast.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")
        elif "dev" in version.lower():
            print_important("You're currently using a developer build of Ghast.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")                
        ...
        print_info(f"Ghast can now be used with {Ghast.command_prefix} prefix.")
        send_notification("Ghast", "Successfully connected!", 10)  
        global __ghastloaded__
        __ghastloaded__ = True
        if __sounds__:
            if str(sounddevice.query_devices()) != "":
    ...
    async def on_message_delete(message):
        if __ghastloaded__:
            ...
            if __ghostpingdetect__:
                if Ghast.user.mentioned_in(message):
    ...
    async def on_message(message):
        if __ghastloaded__:
            ...
            if message.author.id != Ghast.user.id:
                ...
                if __giveawaysniper__:
                    ...
                    if message.author.id in giveawayBots and message.author.bot:
                        ...
                        if isGiveaway:
                            ...
                            #     if f"<@{Ghast.user.id}>" in message.content.lower():
        ...
        await Ghast.process_commands(message)  

    for filename in os.listdir('scripts/'):
        if filename.endswith('.py'):
            include(f'scripts/{filename}')

    @Ghast.command(name="scripts", description="Display all custom scripts.", usage="scripts", aliases=["customscripts"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="customcommands", description="Display all custom commands.", usage="customcommands", aliases=["ccmds"])
...
{__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="allcmds", description="Print a list of all the commands.", usage="allcmds", aliases=["features"])
    async def allcmds(ctx):
        await ctx.message.delete()
        content = ""
        totalCommands = len(Ghast.commands)
        for command in Ghast.commands:
    ...
    @Ghast.command(name="search", description="Search for commands.", usage="search [term]")
...
```""", delete_after=__deletetimeout__)
            ...
            text2 = ""
            searchedItems = 0
            for cmd in Ghast.commands:
                if command in cmd.name or command in cmd.description or command in cmd.aliases:
                    searchedItems += 1
                    text += f"`{Ghast.command_prefix}`**{cmd.usage}** » {cmd.description}\n"
                    text2 += f"{Ghast.command_prefix}{cmd.usage} » {cmd.description}\n"
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="help", description="The help command.", usage="help (command)", aliases=["cmds", "commands"])
    async def help(ctx, *, command = None):
        totalcmds = len(Ghast.commands)-len(scriptsList)
        if command is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
Arguments in `[]` are required, arguments in `()` are optional.

`{Ghast.command_prefix}`**text (page 1/2)** » Text commands.
`{Ghast.command_prefix}`**fun (page 1)** » Fun commands.
`{Ghast.command_prefix}`**image (page 1)** » Image commands.
`{Ghast.command_prefix}`**moderation (page 1)** » Moderation commands.
`{Ghast.command_prefix}`**info (page 1)** » Info commands.
`{Ghast.command_prefix}`**user (page 1)** » User commands.
`{Ghast.command_prefix}`**selfbot (page 1)** » Selfbot commands.
`{Ghast.command_prefix}`**webhook (page 1)** » Webhook commands.
`{Ghast.command_prefix}`**abuse (page 1)** » Abuse commands.
`{Ghast.command_prefix}`**themes (page 1)** » Theme commands.
`{Ghast.command_prefix}`**giveaway (page 1)** » Giveaway commands.
`{Ghast.command_prefix}`**nsfw (page 1)** » NSFW commands.
`{Ghast.command_prefix}`**proxy (page 1)** » Proxy commands.
`{Ghast.command_prefix}`**tools (page 1)** » Discord and other tools.
`{Ghast.command_prefix}`**customcommands** » Your custom commands.
`{Ghast.command_prefix}`**customscripts** » Your scripts.

`{Ghast.command_prefix}`**search [term]** » Search for a command.
`{Ghast.command_prefix}`**help (command)** » Help for a specific command.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="themes", description="Theme related commands.", usage="themes")
    async def themes(ctx):
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Commands", value=f"`{Ghast.command_prefix}`**newtheme [name]** » Create a new theme with the given name.\n`{Ghast.command_prefix}`**deltheme [name]** » Delete the named theme.\n`{Ghast.command_prefix}`**theme [theme]** » Change your current theme.\n`{Ghast.command_prefix}`**ctheme** » Community themes.", inline=False)
...
[ Commands ]
{Ghast.command_prefix}newtheme [name] » Create a new theme with the given name.
{Ghast.command_prefix}deltheme [name] » Delete the named theme.
{Ghast.command_prefix}theme [theme] » Change your current theme.
{Ghast.command_prefix}cthemes » Community themes.


# {__embedfooter__}```""", delete_after=__deletetimeout__)

    @Ghast.command(name="ctheme", description="Community themes.", usage="ctheme", aliases=["communitythemes", "cloudthemes", "cthemes"])
    async def ctheme(ctx, *, dl = None):
        if dl is None:
            url = "https://Ghast.cool/assets/cthemes.txt"
            themes = requests.get(url).text.split("\n")

            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", description=f"Community Themes, run `{Ghast.command_prefix}ctheme (theme name)` to download the theme.\n ", color=__embedcolour__)
...
Community Themes, run {Ghast.command_prefix}ctheme (theme name) to download the theme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
            ...
            request = requests.get("https://Ghast.cool/assets/cthemes.txt")
            themes = []
            ...
            if dl in themes:
                url = f'https://Ghast.cool/assets/cthemes/{dl}.json'
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="text", description="Text related commands.", usage="text (page)")
    async def text(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**js [message]** » Send all your messages in a JavaScript code block.
`{Ghast.command_prefix}`**lua [message]** » Send all your messages in a Lua code block.
`{Ghast.command_prefix}`**php [message]** » Send all your messages in a PHP code block.
`{Ghast.command_prefix}`**html [message]** » Send all your messages in a HTML code block.
`{Ghast.command_prefix}`**css [message]** » Send all your messages in a CSS code block.
`{Ghast.command_prefix}`**yaml [message]** » Send all your messages in a YAML code block.
`{Ghast.command_prefix}`**json [message]** » Send all your messages in a JSON code block.
`{Ghast.command_prefix}`**cpp [message]** » Send all your messages in a C++ code block.
`{Ghast.command_prefix}`**cs [message]** » Send all your messages in a C# code block.
`{Ghast.command_prefix}`**java [message]** » Send all your messages in a Java code block.
`{Ghast.command_prefix}`**python [message]** » Send all your messages in a Python code block.
`{Ghast.command_prefix}`**secret [message]** » Send all your messages in a secret block.
`{Ghast.command_prefix}`**secretletters [message]** » Put all lettes from your message into separate secret blocks
`{Ghast.command_prefix}`**regional [message]** » Replace all letters with emoji.
`{Ghast.command_prefix}`**bold [message]** » Send all your messages in bold.
`{Ghast.command_prefix}`**italic [message]** » Send all your messages in italics.
            """)
                ...
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**rembed (delay) [title]** » Kill Discord's API with a sexy rainbow embedded message.
`{Ghast.command_prefix}`**cembed [title] [description] [colour]** » Create a custom embedded message.
`{Ghast.command_prefix}`**embed [title]** » Create an embedded message.
`{Ghast.command_prefix}`**suggest [suggestion]** » Suggest something.
`{Ghast.command_prefix}`**privatemsg [message]** » Send an encrypted message.
`{Ghast.command_prefix}`**privatemsgdecode [message]** » Decode an encrypted message.
`{Ghast.command_prefix}`**blank** » Send a blank message
`{Ghast.command_prefix}`**length [string]** » Get the length of a string.
`{Ghast.command_prefix}`**chatbypass [text]** » Bypass chat language restrictions.
`{Ghast.command_prefix}`**shrug** » Shrug your arms.
`{Ghast.command_prefix}`**tableflip** » Flip the table.
`{Ghast.command_prefix}`**unflip** » Put the table back.
`{Ghast.command_prefix}`**lmgtfy [search]** » Let me Google that for you.
`{Ghast.command_prefix}`**typing [start/stop]** » Start or stop typing.
`{Ghast.command_prefix}`**aesthetic [text]** » Send your text s p a c e d out.
`{Ghast.command_prefix}`**lowercase [msg]** » Send your message in lowercase.
`{Ghast.command_prefix}`**uppercase [msg]** » Send your message in uppercase.
`{Ghast.command_prefix}`**sentencecase [msg]** » Send your messages in sentence case.
`{Ghast.command_prefix}`**ascii [text]** » Send your message in ascii.
`{Ghast.command_prefix}`**zalgo [text]** » Unleash the zalgo into your message.  
`{Ghast.command_prefix}`**leet [text]** » Turn your text into 1337 text.
`{Ghast.command_prefix}`**fakeedited [message]** » "Edit" a message.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="fun", description="Fun related commands.", usage="fun")
    async def fun(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**slots** » Play the slot machine.
`{Ghast.command_prefix}`**yomomma** » Random yo momma joke.
`{Ghast.command_prefix}`**socialcredit [@user]** » A users social credit score. 
`{Ghast.command_prefix}`**roast [@user]** » Roast a user.
`{Ghast.command_prefix}`**howgay [@user]** » How gay a user is.
`{Ghast.command_prefix}`**howskid [@user]** » Check the percentage of a skid.
`{Ghast.command_prefix}`**iq [@user]** » Check how smart a user is.
`{Ghast.command_prefix}`**pp [@user]** » The length of a user's penis.
`{Ghast.command_prefix}`**rainbowrole [@role]** » Kill Discord's API with a sexy rainbow role.
`{Ghast.command_prefix}`**coinflip** » Flip a coin.
`{Ghast.command_prefix}`**dice** » Roll a dice.
`{Ghast.command_prefix}`**8ball [question]** » Ask the magic eight ball a question.
`{Ghast.command_prefix}`**choice [choice1] [choice2]** » Pick a random choice.
`{Ghast.command_prefix}`**dox [@user]** » Dox the mentioned user.
`{Ghast.command_prefix}`**fakenitro [url]** » Hide a link in a nitro URL.
`{Ghast.command_prefix}`**purgehack** » Purge without permissions.
`{Ghast.command_prefix}`**dadjoke** » A random dad joke.
`{Ghast.command_prefix}`**randommessage** » A random message.
`{Ghast.command_prefix}`**randomquestion** » A random question.
`{Ghast.command_prefix}`**rickroll** » Send never gonna give you up lyrics one by one.
`{Ghast.command_prefix}`**stoprickroll** » Stop sending rick astley lyrics.
`{Ghast.command_prefix}`**countdown [number]** » Count down from a number.
`{Ghast.command_prefix}`**countup [number]** » Count up from a number.
`{Ghast.command_prefix}`**pytoexe [path]** » Convert a PY file to an executable.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="image", description="Image related commands.", usage="image")
    async def image(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**meme** » A random meme.
`{Ghast.command_prefix}`**cat** » A random cat image.
`{Ghast.command_prefix}`**dog** » A random dog image.
`{Ghast.command_prefix}`**shiba** » A random shiba image.
`{Ghast.command_prefix}`**fox** » A random fox image. (Thanks Imf44 <3)
`{Ghast.command_prefix}`**avatar [@user]** » Get the mentioned user's avatar.
`{Ghast.command_prefix}`**servericon** » Get the server's icon. 
`{Ghast.command_prefix}`**achievement ["text"] (icon)** » Create a fake minecraft achievement image.
`{Ghast.command_prefix}`**challenge ["text"] (icon)** » Create a fake minecraft challenge image.
`{Ghast.command_prefix}`**captcha [text]** » Create a fake reCaptcha.
`{Ghast.command_prefix}`**amiajoke [@user]** » Make a user a joke.
`{Ghast.command_prefix}`**didyoumean ["text 1"] ["text 2"]** » Create a google did you mean image.
`{Ghast.command_prefix}`**drake ["text 1"] ["text 2"]** » Create a drake meme image.
`{Ghast.command_prefix}`**facts [text]** » Create a facts meme image.
`{Ghast.command_prefix}`**jokeoverhead [image url]** » Create a joke over head image.
`{Ghast.command_prefix}`**pornhub ["text 1"] ["text 2"]** » Create a pornhub logo image.
`{Ghast.command_prefix}`**salty [@user]** » Make someone salty.
`{Ghast.command_prefix}`**ship [@user 1] [@user 2]** » Ship two people.
`{Ghast.command_prefix}`**supreme [text]** » Create a supreme logo image.
`{Ghast.command_prefix}`**trash [@user]** » Put someone in the trash.
`{Ghast.command_prefix}`**what [image url]** » Make a what meme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="nsfw", description="NSFW related commands.", usage="nsfw")
    async def nsfw(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**boobs** » Pictures or videos of boobs.
`{Ghast.command_prefix}`**ass** » Pictures or videos of ass.
`{Ghast.command_prefix}`**pussy** » Pictures or videos of pussy.
`{Ghast.command_prefix}`**porngif** » Porn gifs.
`{Ghast.command_prefix}`**hentai** » Pictures or videos of hentai.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="moderation", description="Moderation related commands.", usage="moderation")
    async def moderation(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**ban [@user]** » Ban the mentioned user.
`{Ghast.command_prefix}`**unban [id]** » Unban the mentioned id.
`{Ghast.command_prefix}`**banlist** » See the server's ban list.
`{Ghast.command_prefix}`**kick [@user]** » Kick the mentioned user.
`{Ghast.command_prefix}`**mute [@user]** » Mute the menitioned user.
`{Ghast.command_prefix}`**unmute [@user]** » Unmute the mentioned user.
`{Ghast.command_prefix}`**newrole [name]** » Create a new role.
`{Ghast.command_prefix}`**delrole [@role]** » Delete the mentioned role.
`{Ghast.command_prefix}`**purge [amount]** » Purge X amount of messages.
`{Ghast.command_prefix}`**lock** » Lock the command channel.
`{Ghast.command_prefix}`**unlock** » Unlock the command channel.
`{Ghast.command_prefix}`**lockdown** » Lock the entire server.
`{Ghast.command_prefix}`**unlockdown** » Unlock the entire server.
`{Ghast.command_prefix}`**spacechannel [channel name]** » Create a channel with spaces.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="info", description="Info related commands.", usage="info")
    async def info(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**userinfo [@user]** » Information about the mentioned user.
`{Ghast.command_prefix}`**serverinfo** » Information about the command server.
`{Ghast.command_prefix}`**watchdogstats** » Get stats about Hypixel's Anticheat, Watchdog.
`{Ghast.command_prefix}`**getmessage [message id]** » Get a message by ID.
`{Ghast.command_prefix}`**geoip [ip]** » Get information from an IP address.
`{Ghast.command_prefix}`**ping [ip/domain]** » Ping a domain or ip address. 
`{Ghast.command_prefix}`**crypto [currency]** » Get the current data on a cryptocurrency.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="user", description="User related commands.", usage="user")
    async def user(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**purgeself [amount]** » Purge your messages.
`{Ghast.command_prefix}`**statuscycle** » Start a custom status cycle.
`{Ghast.command_prefix}`**statuscycletext [text]** » Set the text used in status cycle.
`{Ghast.command_prefix}`**clearstatus** » Clear your status.
`{Ghast.command_prefix}`**nickname [text]** » Set your nickname to anything.
`{Ghast.command_prefix}`**clearnickname** » Clear your nickname.
`{Ghast.command_prefix}`**ppin [message id]** » Add a message to your personal pins.
`{Ghast.command_prefix}`**ppins** » List all your pinned messages.
`{Ghast.command_prefix}`**ppindel [pin id]** » Delete a pin from your personal pins.
`{Ghast.command_prefix}`**backupfriends** » Backup all your friend's user IDs to a file.
`{Ghast.command_prefix}`**backupservers** » Backup all your servers and try to create invites for each one.
`{Ghast.command_prefix}`**changehypesquad [bravery/brilliance/balance]** » Change your hypesquad house.
`{Ghast.command_prefix}`**stealpfp [@user]** » Set someones avatar as your avatar.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="selfbot", description="Selfbot related commands.", usage="selfbot")
    async def selfbot(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**afkmode** » Toggle afk mode.
`{Ghast.command_prefix}`**settings** » The bot's settings.
`{Ghast.command_prefix}`**restart** » Restart Ghast selfbot.
`{Ghast.command_prefix}`**prefix [prefix]** » Set the command prefix.
`{Ghast.command_prefix}`**dumpchat [amount] (channel id) (oldest first, true/false)** » Get the chat's history.
`{Ghast.command_prefix}`**invite** » Get Ghast's Discord server invite link.
`{Ghast.command_prefix}`**addccmd [name] [response]** » Add a custom command.
`{Ghast.command_prefix}`**enablesniper [type, nitro/privnote/giveaway]** » Enable a sniper.
`{Ghast.command_prefix}`**disablesniper [type, nitro/privnote/giveaway]** » Disable a sniper.
`{Ghast.command_prefix}`**enabledetect [type, selfbot/..]** » Enable a detection.
`{Ghast.command_prefix}`**disabledetect [type, selfbot/..]** » Disable a detection.
`{Ghast.command_prefix}`**riskmode** » Disable and enable risk mode
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="webhook", description="Webhook related commands.", usage="webhook")
    async def webhook(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**delwebhook [id]** » Delete a webhook from the ID.
`{Ghast.command_prefix}`**newwebhook [name]** » Create a webhook in the command channel.
`{Ghast.command_prefix}`**spamwebhook [amount] [url] (message)** » Spam the shit out of a webhook.
`{Ghast.command_prefix}`**webhooksetup** » Creates a new server with webhooks.
`{Ghast.command_prefix}`**webhookinfo [id]** » Information about the webhook.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="abuse", description="Abuse related commands.", usage="abuse")
    async def abuse(ctx, page:int = 1):
        if __riskmode__:
            if __embedmode__:
                if page == 1:
                    embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**spam [amount] [delay] [message]** » Spam X amount of times.
`{Ghast.command_prefix}`**stopspam** » Stop spamming messages.
`{Ghast.command_prefix}`**dmspam [amount] [delay] [@user] [message]** » Spam DM messages X amount of times.
`{Ghast.command_prefix}`**channelspam [amount] [delay] [message]** » Spam a message X amount of times in every channel.
`{Ghast.command_prefix}`**threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]** » Spam create threads with a starting message.
`{Ghast.command_prefix}`**ttsspam [amount] [delay] [message]** » Spam TTS messages X amount of times.
`{Ghast.command_prefix}`**reactspam [emoji] [messages]** » Spam reactions on X amount of messages.
`{Ghast.command_prefix}`**massghostping [delay] [@user]** » Ghost Ping the user in every channel.
`{Ghast.command_prefix}`**ghostping [@user]** » Ping a user then delete the message.
`{Ghast.command_prefix}`**massping (amount of messages) (send delay)** » Ping a mass amount of people in the command server.
`{Ghast.command_prefix}`**massnick [nickname]** » Change the nickname of all members in the command server.
`{Ghast.command_prefix}`**massdm [delay] [amount] [message]** » Send a DM message to everyone in the server.
`{Ghast.command_prefix}`**nukeserver** » Delete all roles and channels in the command server.
`{Ghast.command_prefix}`**destroyserver** » Completely destroy the command server.
`{Ghast.command_prefix}`**deletechannels** » Delete all of the command server's channels.
`{Ghast.command_prefix}`**deleteroles** » Delete all of the command server's roles.
`{Ghast.command_prefix}`**spamchannels [amount] (name)** » Spam create channels with a desired name. (Thanks Port <3)
`{Ghast.command_prefix}`**spamroles [amount] (name)** » Spam create roles with a desired name.
`{Ghast.command_prefix}`**raidjoin [delay] [invite]** » Make all your account tokens join a server.
`{Ghast.command_prefix}`**tokenraid [amount] [channel id] (message)** » Raid a server with all your account tokens.
`{Ghast.command_prefix}`**massban** » Ban all the members in the command server.
`{Ghast.command_prefix}`**masskick** » Kick all the members in the command server.
                """)
...
{Ghast.command_prefix}stopspam » Stop spamming messages.
{Ghast.command_prefix}dmspam [amount] [delay] [@user] [message] » Spam DM messages X amount of times.
{Ghast.command_prefix}channelspam [amount] [delay] [message] » Spam X amount of times in all channels.
{Ghast.command_prefix}threadspam [delay] [amount] [name] [startmessage] » Spam create threads with a starting message.
{Ghast.command_prefix}ttsspam [amount] [delay] [message] » Spam TTS messages X amount of times.
{Ghast.command_prefix}reactspam [emoji] [messages] » Spam reactions on X amount of messages.
{Ghast.command_prefix}massghostping [delay] [@user] » Ghost Ping the user in every channel.
{Ghast.command_prefix}ghostping [@user] » Ping a user then delete the message.
{Ghast.command_prefix}massping » Ping a mass amount of people in the command server.
{Ghast.command_prefix}massnick [nickname] » Change the nickname of all members in the command server.
{Ghast.command_prefix}massdm [delay] [amount] [message] » Send a DM message to everyone in the server.
{Ghast.command_prefix}nukeserver » Delete all roles and channels in the command server.
{Ghast.command_prefix}destroyserver » Completely destroy the command server.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                          
    ...
    @Ghast.command(name="tools", description="Discord and other tools.", usage="tools")
    async def tools(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**tokeninfo [token]** » Information about a token. 
`{Ghast.command_prefix}`**nuketoken [token]** » Nuke a token.
`{Ghast.command_prefix}`**checktoken [token]** » Checks if a token is working.
`{Ghast.command_prefix}`**checktokens** » Check your tokens. 
`{Ghast.command_prefix}`**nitrogen** » Generate a nitro code.
`{Ghast.command_prefix}`**tokengen** » Generate a discord user token.
`{Ghast.command_prefix}`**identitygen** » Generate a fake identity.
`{Ghast.command_prefix}`**passwordgen [length]** » Generate a secure password.
`{Ghast.command_prefix}`**ccgen** » Generate a fake Credit card.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="giveaway", description="Giveaway related commands.", usage="giveaway")
...
`{Ghast.command_prefix}`**gstart [duration] [winners] [prize]** » Start a giveaway in the same channel
`{Ghast.command_prefix}`**gend [message id]** » End a giveaway
`{Ghast.command_prefix}`**greroll [message id]** » Re-roll a giveaway
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="proxy", description="Proxy related commands.", usage="proxy")
    async def proxy(ctx, page:int=1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}",
                                    color=__embedcolour__, description=f"""
`{Ghast.command_prefix}`**proxies http** » Scrape HTTP proxies.
`{Ghast.command_prefix}`**proxies https** » Scrape HTTPS proxies.
`{Ghast.command_prefix}`**proxies socks4** » Scrape SOCKS4 proxies.
`{Ghast.command_prefix}`**proxies socks5** » Scrape SOCKS5 proxies.
`{Ghast.command_prefix}`**proxies all** » Scrape HTTP, HTTPS, SOCKS4 AND SOCKS5 proxies.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="hiddenchannels", description="Sends a list of all the channels you cant see.", usage="hiddenchannels (guild id)")
    async def hiddenchannels(ctx, guild=None):
        if guild == None:
            guild = ctx.guild
        else:
            guild = await Ghast.fetch_guild(int(guild))
        hiddenChannels = []
...
```""", delete_after=__deletetimeout__) 
    ...
    @Ghast.command(name="clearconsole", description="Clear your console.", usage="clearconsole", aliases=["resetconsole", "consoleclear", "consolereset"])
    ...
    @Ghast.command(name="blocksend", description="Send a message to a blocked user.", usage="blocksend [user id] [messages]", aliases=["sendblocked", "sendtoblocked"])
    async def blocksend(ctx, userid:int, *, message):
        user = await Ghast.fetch_user(userid)
        await user.unblock()
    ...
    @Ghast.command(name="riskmode", description="Disable and enable risk mode", usage="riskmode")
    ...
    @Ghast.command(name="embedmode", description="Toggle embed mode.", usage="embedmode")
    ...
    @Ghast.command(name="textmode", description="Toggle text mode.", usage="textmode")
    ...
    @Ghast.command(name="readall", description="Mark every message as read.", usage="readall")
    async def readall(ctx):
        index = 0
        index2 = 0
        DiscumClient = discum.Client(token=__token__, log=False)
        for guild in Ghast.guilds:
            messages2 = []
    ...
    @Ghast.command(name="specs", description="Your computers specifications.", usage="specs", aliases=["computerspecs", "pcspecs", "specifications"])
    async def specs(ctx):
...
```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="crypto", description="Get the current data on a cryptocurrency.", usage="crypto [currency]", aliases=["cryptodata"])
...
```""")
    ...
    @Ghast.command(name="proxies", description="Scrape an type of proxy.", usage="proxies [http, https, socks4, socks5, all]", aliases=["proxygen", "genproxies"])
    ...
    @Ghast.command(name="stealpfp", description="Set someones avatar as your avatar.", usage="stealpfp [@user]", aliases=["stealavatar"])
    ...
    # @Ghast.command(name="stealusername", description="Steal someones username.", usage="stealusername [@user]", aliases=["stealname"])
    ...
    # @Ghast.command(name="stealprofile", description="Steal someones avatar and username.", usage="stealprofile [@user]")
    ...
    @Ghast.command(name="cloneemoji", description="Clone an emoji to the command server.", usage="cloneemoji [emoji]", aliases=["stealemoji"])
    ...
    @Ghast.command(name="enabledetect", description="Enable a detection.", usage="enabledetect [type, selfbot/ghostping/bans/deletedmessages/webhookmodification]")
    ...
    @Ghast.command(name="disabledetect", description="Disable a detection.", usage="disabledetect [type, selfbot/ghostping/bans/deletedmessages/webhookmodification]")
    ...
    @Ghast.command(name="enablesniper", description="Enable a sniper.", usage="enablesniper [type, nitro/privnote/giveaway/tickets]")
    ...
    @Ghast.command(name="disablesniper", description="Disable a sniper.", usage="disablesniper [type, nitro/privnote/giveaway/tickets]")
...
#             @Ghast.command(name="ghastusers", description="Finds all the people using Ghast in a server.", usage="ghastusers")
#             @commands.guild_only()
#             async def ghastusers(ctx):
#                 message = await ctx.send("Looking for people that have Ghast, this may take a while...")
...
#                     ghastguild = await Ghast.fetch_guild(838869729829191681)
...
#                     embed=discord.Embed(
#                         title="Ghast Users",
#                         description=f"There are a total of `{len(ghastUsers)}` Ghast users in `{ctx.guild.name}`\n \n```\n" + ", ".join(ghastUsers) + f"\n```",
#                         color=__embedcolour__
#                     )
#                     embed.set_thumbnail(url=__embedimage__)
#                     embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
...
# There is a total of {len(ghastUsers)} in {ctx.guild.name}.
...
# ```""")
    ...
    @Ghast.command(name="addccmd", description="Add a custom command.", usage="addccmd [name] [response]", aliases=["addcustomcommand"])
    async def addccmd(ctx, name, *, response):
        ...
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Added `{Ghast.command_prefix}{name}` to your custom commands.", delete_after=__deletetimeout__)

    @Ghast.command(name="delccmd", description="Remove a custom command.", usage="delccmd [name]", aliases=["deletecustomcommand", "delcustomcommand", "removecustomcommand", "removeccmd", "deleteccmd"])
    async def delccmd(ctx, name):
        ...
        json.dump(customCommands, open("customcommands.json", "w"), indent=4, sort_keys=False)
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Removed `{Ghast.command_prefix}{name}` from your custom commands", delete_after=__deletetimeout__)            

    @Ghast.command(name="boobs", description="Pictures or videos of boobs.", usage=f"boobs", aliases=["tits", "tit", "milkers", "titties", "boob"])
    async def boobs(ctx):
        type = "boobs"
        image = get_nsfw(type)
    ...
    @Ghast.command(name="ass", description="Pictures or videos of ass.", usage=f"ass")
    async def ass(ctx):
        type = "ass"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @Ghast.command(name="pussy", description="Pictures or videos of pussy.", usage=f"pussy")
    async def pussy(ctx):
        type = "pussy"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @Ghast.command(name="porngif", description="Porn gifs.", usage=f"porngif")
    ...
    @Ghast.command(name="hentai", description="Pictures or videos of hentai.", usage=f"hentai")
    ...
    @Ghast.command(name="discordtheme", description="Change default Discord theme.", usage="discordtheme [light/dark]")
    ...
    @Ghast.command(name="changehypesquad", description="Change your hypesquad house.", usage="changehypesquad [bravery/brilliance/balance]")
    ...
    @Ghast.command(name="backupfriends", description="Backup all your friend's user IDs to a file.", usage="backupfriends", aliases=["friendbackup"])
    ...
    @Ghast.command(name="backupservers", description="Backup all your servers and try to create invites for each one.", usage="backupservers", aliases=["backupguilds", "serverbackup", "guildbackup"])
    async def backupservers(ctx):
        ...
        for item in guilds:
            ...
            server = discord.utils.get(Ghast.guilds, id=int(guildid))
    ...
    @Ghast.command(name="richpresence", description="Enable or disable rich presence.", usage="richpresence [on/off]", aliases=["rpc"])
    ...
    @Ghast.command(name="spacechannel", description="Create a channel with spaces.", usage="spacechannel [channel name]")
...
```""")
    ...
    @Ghast.command(name="uwu", description="Translate your messages to uwu!", usage="uwu [message]")
    async def uwu__(ctx, *, message):
        uwued = uwuify.uwu(message)
        await ctx.send(uwued)
        
    @Ghast.command(name="uwuify", description="Automatically translate all your sent messages to uwu!", usage="uwuify")
    ...
    @Ghast.command(name="geoip", description="Get information from an IP address.", usage="geoip [ip]", aliases=["iplookup", "lookupip", "ipinfo"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @Ghast.command(name="invite", description="Get Ghast's Discord server invite link.", usage="invite")
    async def invite(ctx):
        print_info(f"Discord server invite: {discordServer}")

    @Ghast.command(name="pytoexe", description="Convert a PY file to an executable.", usage="pytoexe [path]", aliases=["pythontoexe", "py2exe", "python2exe"])
...
```""")          
    ...
    @Ghast.command(name="statuscycle", description="Start a custom status cycle.", usage="statuscycle", aliases=["cyclestatus"])
    async def statuscycle(ctx):
        ...
        Ghast.loop.create_task(loopStatus(cycleStatusText))
    ...
    @Ghast.command(name="statuscycletext", description="Set the text used in status cycle.", usage="statuscycletext [text]", aliases=["cyclestatustext"])
    ...
    @Ghast.command(name="ghostping", description="Ping a user then delete the message.", usage="ghostping [@user]")
    async def ghostping(ctx, user: discord.User):
        pass

    @Ghast.command(name="getmessage", description="Get a message by ID.", usage="getmessage [message id]", aliases=["fetchmessage"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @Ghast.command(name="watchdogstats", description="Get stats about Hypixel's Anticheat, Watchdog", usage="watchdogstats")
...
```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="ppin", description="Add a message to your personal pins.", usage="ppin [message id]", aliases=["personalpin", "addppin", "addpersonalpin"])
    ...
    @Ghast.command(name="ppins", description="List all your pinned messages.", usage="ppins", aliases=["personalpins"])
    async def ppins(ctx):
        data = json.load(open("data/personal-pins.json"))
    ...
    @Ghast.command(name="ppindel", description="Delete a pin from your personal pins.", usage="ppindel [pin id]", aliases=["ppindelete", "removeppin", "deleteppin", "personalpindelete", "deletepersonalpin", "removepersonalpin"])
    ...
    @Ghast.command(name="countdown", description="Count down from a number.", usage="countdown [number]")
    async def countdown(ctx, number: int):
        for count in range(number, 0, -1):
            await ctx.send(count)

    @Ghast.command(name="countup", description="Count up from a number.", usage="countup [number]")
    async def countup(ctx, number: int):
        for count in range(number):
            await ctx.send(count)

    @Ghast.command(name="massban", description="Ban all the members in the command server.", usage="massban")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="masskick", description="Kick all the members in the command server.", usage="masskick")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="raidjoin", description="Make all your account tokens join a server.", usage="raidjoin [delay] [invite]")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="tokenraid", description="Raid a server with all your account tokens.", usage="tokenraid [threads] [amount] [channel id] (message)")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @Ghast.command(name="checktoken", description="Checks if a token is working.", usage="checktoken [token]")
    ...
    @Ghast.command(name="checktokens", description="Checks if your tokens are working.", usage="checktokens")
    ...
    @Ghast.command(name="wipetoken", description="Completely wipe a token.", aliases=["cleantoken"])
    ...
    @Ghast.command(name="nuketoken", description="Nuke a token.", usage="nuketoken [token]", aliases=["tokennuke"])
    ...
    @Ghast.command(name="gstart", description="Start a giveaway in the same channel", usage="gstart [duration] [winners] [prize]", aliases=["giveawaystart", "startgiveaway"])
            ...
            if winners is not None:
                if prize is not None:
                    ...
                    if "🎉 **GIVEAWAY** 🎉" in msg.content:
                        ...
                        entries.remove(f"<@{Ghast.user.id}>")
                    ...
                    await ctx.send(
                        f"❌ **Incorrect Syntax**\nTry: `{Ghast.command_prefix}gstart 30m 1 Awesome T-Shirt`")
            else:
    ...
    @Ghast.command(name="gend", description="End a giveaway", usage="gend [message id]", aliases=["giveawayend", "endgiveaway"])
    async def gend(ctx, id: int = None):
        ...
        #print("Bot user ID: " + str(Ghast.user.id))

        if msgId == id and msgAuthorId == Ghast.user.id:
    ...
    @Ghast.command(name="greroll", description="Re-roll a giveaway", usage="greroll [message id]", aliases=["giveawayreroll", "rerollgiveaway"])
    async def greroll(ctx, id: int = None):
        ...
        if msg.author.id == Ghast.user.id:
            if "🎉 **GIVEAWAY** 🎉" in msg.content:
                #running = True
    ...
    @Ghast.command(name="typing", description="Start or stop typing.", usage="typing [start/stop]", aliases=["inftyping", "infintetyping"])
    ...
    @Ghast.command(name="sounds", description="Toggle notification sounds.", usage="sounds", aliases=["togglesounds", "soundstoggle"])
    async def sounds(ctx):
        ...
        if __sounds__:
            ...
            cfg["sounds"] = False
            Config.saveConfig(cfg)
            await ctx.send("Disabled notification sounds.")
        else:
            __sounds__ = True
            cfg = Config.getConfig()
            cfg["sounds"] = True
            Config.saveConfig(cfg)
            await ctx.send("Enabled notification sounds.")

    @Ghast.command(name="ping", description="Ping a domain or ip address.", usage="ping [ip/domain]")
...
```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="cloneserver", description="Clone a server.", usage="cloneserver", aliases=["copyserver"])
    async def cloneserver(ctx):
        serverName = ctx.guild.name
        serverIcon = ctx.guild.icon

        newGuild = await Ghast.create_guild(serverName)
    ...
    @Ghast.command(name="webhooksetup", description="Create a new server with webhooks.", usage="webhooksetup", aliases=["setupwebhooks"])
    async def webhooksetup(ctx):
        ...
        guild = await Ghast.create_guild(ctx.author.name + "'s webhooks")
    ...
    @Ghast.command(name="spamwebhook", description="Spam the shit out of a webhook.", usage="spamwebhook [amount] [url] (message)")
    ...
    @Ghast.command(name="newwebhook", description="Create a webhook in the command channel.", usage="newwebhook [name]", aliases=["createwebhook"])
    ...
    @Ghast.command(name="delwebhook", description="Delete a webhook from the ID.", usage="delwebhook [id]", aliases=["deletewebhook", "removewebhook"])
    async def delwebhook(ctx, id: int):
        webhook = await Ghast.fetch_webhook(id)
    ...
    @Ghast.command(name="webhookinfo", description="Information about the webhook.", usage="webhookinfo [id]", aliases=["webhooklookup", "lookupwebhook"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="dumpchat", description="Get the chat's history.", usage="dumpchat [amount] (channel id) (oldest first, true/false)", aliases=["savechat", "chathistory"])
    async def dumpchat(ctx, amount: int, channelId: int = None, oldestFirst: bool = False):
        if channelId is None:
            ...
            channel = Ghast.get_channel(channelId)
    ...
    @Ghast.command(name="newtheme", description="Create a new theme with the given name.", usage="newtheme [name]", aliases=["createtheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="deltheme", description="Delete the named theme.", usage="deltheme [name]", aliases=["deletetheme", "removetheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="theme", description="Change your current theme.", usage="theme [theme]", aliases=["settheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="prefix", description="Set the command prefix.", usage="prefix [prefix]", aliases=["c"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="restart", description="Restart Ghast selfbot.", usage="restart", aliases=["reboot", "reload"])
    async def restart(ctx):
        print_info("Restarting Ghast...")
        await ctx.send("Restarting Ghast...")
        restart_bot()

    @Ghast.command(name="firstmessage", description="Get the first message in the command channel.", usage="firstmessage")
    ...
    #@Ghast.command(name="haste", description="Upload text to Ghast's Haste site.", usage="haste [text]")
    #async def haste(ctx, *, text):
    #    url = "https://haste-server.letmewoosh.repl.co"
    ...
    @Ghast.command(name="shrug", description="Shrug your arms.", usage="shrug")
    async def shrug(ctx):
        await ctx.send(f"¯\_(ツ)_/¯")

    @Ghast.command(name="tableflip", description="Flip the table.", usage="tableflip")
    async def tableflip(ctx):
        await ctx.send("(╯°□°）╯︵ ┻━┻")

    @Ghast.command(name="unflip", description="Put the table back.", usage="unflip")
    async def unflip(ctx):
        await ctx.send("┬─┬ ノ( ゜-゜ノ)")

    # @Ghast.command(name="hide", description="Hide a message behind another message.", usage="hide [msg1] [msg2]")
    # async def hide(ctx, msg1, msg2):
    #     await ctx.send(msg1+hideText+msg2)

    @Ghast.command(name="blank", description="Send a blank message", usage="blank")
    async def blank(ctx):
        await ctx.send("** **")

    @Ghast.command(name="length", description="Get the length of a string.", usage="length [string]", aliases=["stringlength"])
    async def length(ctx, *, string):
        await ctx.send(f"Length of `{string}`: " + len(string))

    @Ghast.command(name="lmgtfy", description="Let me Google that for you.", usage="lmgtfy [search]", aliases=["letmegooglethatforyou"])
    async def lmgtfy(ctx, *, search):
        await ctx.send(f"https://lmgtfy.app/?q={search.replace(' ', '+')}")

    @Ghast.command(name="selfbotcheck", description="Checks for users using a selfbot.", usage="selfbotcheck")
    ...
    @Ghast.command(name="nukeserver", description="Delete all roles and channels in the command server.", usage="nukeserver", aliases=["nukeguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="destroyserver", description="Completely destroy the command server.", usage="destroyserver", aliases=["destroyguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="spamchannels", description="Spam create channels with a desired name. (Thanks Port <3)", usage="spamchannels [amount] (name)", aliases=["spamcreatechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @Ghast.command(name="spamroles", description="Spam create roles with a desired name.", usage="spamroles [amount] (name)", aliases=["spamcreateroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @Ghast.command(name="deletechannels", description="Delete all of the command server's channels.", usage="deletechannels", aliases=["delchannels", "removechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="deleteroles", description="Delete all of the command server's roles.", usage="deleteroles", aliases=["delroles", "removeroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)      
    ...
    @Ghast.command(name="dmspam", description="Spam DM messages X amount of times.", usage="dmspam [amount] [delay] [@user] [message]", aliases=["spamdm"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="threadspam", description="Spam create threads with a starting message.", usage="threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]", aliases=["spamthreads", "spamcreatethreads"])
    async def threadspam(ctx, delay: int, amount: int, addusers: bool, name: str = "Ghast best selfbot!", *, startmessage: str):
        if __riskmode__:
            users = []
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="channelspam", description="Spam a message X amount of times in every channel.", usage="channelspam [amount] [delay] [message]", aliases=["sendall", "sendtoallchannels", "msgallchannels", "messageallchannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @Ghast.command(name="spam", description="Spam X amount of times.", usage="spam [amount] [delay] [message]")
            ...
            async def spamMessages():
                for _ in range(amount):
                    if spammingMessages == True:
                        ...
                        return
            Ghast.loop.create_task(spamMessages())
        else:
            if __embedmode__:
                embed = discord.Embed(title=f"Abusive Commands", color=__embedcolour__, description=f"You have risk mode disabled, you cant use this command.")
                embed.set_thumbnail(url=__embedimage__)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                      
    ...
    @Ghast.command(name="stopspam", description="Stop spamming messages.", usage="stopspam")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @Ghast.command(name="ttsspam", description="Spam TTS messages X amount of times.", usage="ttsspam [amount] [delay] [message]", aliases=["texttospeachspam"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="massghostping", description="Ping a mass amount of people in the command server and delete the messages.", usage="massghostping (amount of messages) (send delay)", aliases=["massghostmention", "theotherfunny"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="massping", description="Ping a mass amount of people in the command server.", usage="massping (amount of messages) (send delay)", aliases=["massmention", "sigmainstaller", "hahafunny"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="massdm", description="Send a DM message to everyone in the server.", usage="massdm [delay] [amount] [message]")
    @commands.guild_only()
    async def massdm(ctx, delay:int=0, amount:int=10, *, message:str):
        if __riskmode__:
            ...
            for _ in range(amount):
                for member in members:
                    try:
                        member = await Ghast.fetch_user(int(member))
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @Ghast.command(name="rickroll", description="Send never gonna give you up lyrics one by one.", usage="rickroll")
    async def rickroll(ctx):
        ...
        Ghast.loop.create_task(sendLyrics())

    @Ghast.command(name="stoprickroll", description="Stop sending rick astley lyrics.", usage="stoprickroll")
    async def stoprickroll(ctx):
        global rickRollEnabled
        rickRollEnabled = False

    @Ghast.command(name="suggest", description="Suggest something.", usage="suggest [suggestion]")
...
# {ctx.author.name} suggested.```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="massnick", description="Change the nickname of all members in the command server.", usage="massnick [nickname]", aliases=["massnickname", "masschangenickname"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="massunnick", description="Reset the nickname of all members in the command server.", usage="massunnick", aliases=["massremovenickname", "massunnickname"])
    ...
    @Ghast.command(name="dadjoke", description="A random dad joke.", usage="dadjoke")
    ...
    @Ghast.command(name="randomquestion", description="A random question.", usage="randomquestion", aliases=["ranquestion"])
    ...
    @Ghast.command(name="randommessage", description="A random message.", usage="randommessage", aliases=["ranmessage"])
    ...
    @Ghast.command(name="meme", description="A random meme.", usage="meme", aliases=["randommeme"])
    async def meme(ctx):
    ...
    @Ghast.command(name="gif", description="Search for a gif.", usage="gif [search]", aliases=["searchgif"])
    ...
    @Ghast.command(name="cat", description="A random cat image.", usage="cat", aliases=["randomcat"])
    async def cat(ctx):
    ...
    @Ghast.command(name="catgif", description="A random cat gif.", usage="catgif", aliases=["randomcatgif"])
    ...
    @Ghast.command(name="dog", description="A random dog image.", usage="dog", aliases=["randomdog"])
    async def dog(ctx):
    ...
    @Ghast.command(name="shiba", description="A random shiba image.", usage="shiba", aliases=["randomshiba"])
    async def shiba(ctx):
    ...
    @Ghast.command(name="fox", description="A random fox image. (Thanks Imf44 <3)", usage="fox", aliases=["randomfox"])
    async def fox(ctx):
    ...
    @Ghast.command(name="achievement", description="Create a fake minecraft achievement image.", usage='achievement ["text"] (icon)', aliases=["minecraftachievement"])
    ...
    @Ghast.command(name="challenge", description="Create a fake minecraft challenge image.", usage='challenge ["text"] (icon)', aliases=["minecraftchallenge"])
    ...
    @Ghast.command(name="captcha", description="Create a fake reCaptcha.", usage="captcha [text]", aliases=["fakecaptcha"])
    ...
    @Ghast.command(name="amiajoke", description="Make a user a joke.", usage="amiajoke [@user]", aliases=["amiajoketoyou"])
    ...
    @Ghast.command(name="didyoumean", description="Create a google did you mean image.", usage='didyoumean ["text 1"] ["text 2"]', aliases=["googledidyoumean"])
    async def didyoumean(ctx, text1="Nighty", text2="Ghast"):
        if CONFIG["api_keys"]["alexflipnote"] == "":
            if __embedmode__:
                embed = discord.Embed(description="This command requires an alexflipnote API key.", color=__embedcolour__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
    ...
    @Ghast.command(name="drake", description="Create a drake meme image.", usage='drake ["text 1"] ["text 2"]', aliases=["drakememe"])
    async def drake(ctx, text1="Nighty Selfbot", text2="Ghast Selfbot"):
        if CONFIG["api_keys"]["alexflipnote"] == "":
            if __embedmode__:
                embed = discord.Embed(description="This command requires an alexflipnote API key.", color=__embedcolour__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
    ...
    @Ghast.command(name="facts", description="Create a facts meme image.", usage='facts [text]', aliases=["factsmeme"])
    ...
    @Ghast.command(name="jokeoverhead", description="Create a joke over head image.", usage="jokeoverhead [image url]")
    ...
    @Ghast.command(name="pornhub", description="Create a pornhub logo image.", usage='pornhub ["text 1"] ["text 2"]')
    async def pornhub(ctx, text1="Ghast", text2="Selfbot"):
        if CONFIG["api_keys"]["alexflipnote"] == "":
            if __embedmode__:
                embed = discord.Embed(description="This command requires an alexflipnote API key.", color=__embedcolour__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
    ...
    @Ghast.command(name="salty", description="Make someone salty.", usage="salty [@user]")
    ...
    @Ghast.command(name="ship", description="Ship two people.", usage="ship [@user 1] [@user 2]")
    ...
    @Ghast.command(name="supreme", description="Create a supreme logo image.", usage='supreme [text]')
    ...
    @Ghast.command(name="trash", description="Put someone in the trash.", usage='trash [@user]')
    async def trash(ctx, user: discord.User):
        if CONFIG["api_keys"]["alexflipnote"] == "":
            ...
            face = avatarUrl(Ghast.user.id, Ghast.user.avatar)
    ...
    @Ghast.command(name="what", description="Make a what meme.", usage='what [image url]')
    ...
    @Ghast.command(name="purgehack", description="Purge without permissions.", usage="purgehack")
    async def purgehack(ctx):
        await ctx.send(f"** **\n"*100)

    @Ghast.command(name="iq", description="Check how smart a user is.", usage="iq [@user]")
    ...
    @Ghast.command(name="howskid", description="Check the percentage of a skid.", usage="howskid [item]")
    ...
    @Ghast.command(name="howgay", description="How gay a user is.", usage="howgay [@user]")
    ...
    @Ghast.command(name="slots", description="Play the slot machine.", usage="slots")
    async def slots(ctx):
...
```""")                               
    ...
    @Ghast.command(name="socialcredit", description="A users social credit score.", usage="socialcredit [@user]")
    ...
    @Ghast.command(name="roast", description="Roast a user.", usage="roast [@user]", aliases=["insult"])
    ...
    @Ghast.command(name="yomomma", description="Random yo momma joke.", usage="yomomma", aliases=["mom", "mum"])
    ...
    @Ghast.command(name="fakeedited", description='"Edit" a message.', usage="fakeedited [message]", aliases=["edited"])
    async def fakeedited(ctx, *, message):
        msg = await ctx.send(message)
        await msg.edit(content=message + " hehe")
        await msg.edit(content=message)

    @Ghast.command(name="pp", description="The length of a user's penis.", usage="pp (@user)", aliases=["dicksize", "cocksize", "penissize"])
    async def pp(ctx, user: discord.User = None):
        size = "8" + "="*random.randint(1, 12) + "D"
        if user is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{Ghast.user.name}'s pp is {size}", color=__embedcolour__)
                embed.set_thumbnail(url=__embedimage__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
                embed.timestamp = datetime.now()
                await ctx.send(embed=embed)
            else:
                await ctx.send(f"{Ghast.user.name}'s pp size\n{size}")
        else:
            if __embedmode__:
    ...
    # @Ghast.command(name="trumptweet", description="Make Donald Trump tweet anything.", usage="trumptweet [tweet]")
    ...
    @Ghast.command(name="rainbowrole", description="Kill Discord's API with a sexy rainbow role.", usage="rainbowrole [@role]")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="rembed", description="Kill Discord's API with a sexy rainbow embedded message.", usage="rembed [text]", aliases=["rainbowembed"])
    ...
    @Ghast.command(name="coinflip", description="Flip a coin.", usage="coinflip", aliases=["flipacoin"])
    ...
    @Ghast.command(name="dice", description="Roll a dice.", usage="dice", aliases=["rolladice"])
    async def dice(ctx):
    ...
    @Ghast.command(name="rps", description="Rock, paper, scissors.", usage="rps", aliases=["rockpaperscissors"])
    ...
    @Ghast.command(name="8ball", description="Ask the magic eight ball a question.", usage="8ball [question]", aliases=["eightball", "magic8ball"])
    ...
    @Ghast.command(name="choice", description="Pick a random choice.", usage="choice [choice1] [choice2]", aliases=["pick"])
    ...
    # @Ghast.command(name="wyr", description="Would you rather questions.", usage="wyr")
    # async def wyr_(ctx):
    #     question, _ = wyr()
    ...
    @Ghast.command(name="dox", description="Dox the mentioned user.", usage="dox [@user]")
    ...
    @Ghast.command(name="fakenitro", description="Hide a link in a nitro URL.", usage="fakenitro [url]")
    ...
    @Ghast.command(name="nitrogen", description="Generate a nitro code.", usage="nitrogen", aliases=["nitrogenerate", "generatenitro", "gennitro"])
    ...
    @Ghast.command(name="tokengen", description="Generate a discord user token.", usage="tokengen", aliases=["generatetoken", "tokengenerate", "gentoken"])
    ...
    @Ghast.command(name="identitygen", description="Generate a fake identity.", usage="identitygen", aliases=["identitygenerate", "generateidentity", "genidentity"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="passwordgen", description="Generate a secure password.", usage="passwordgen [length]", aliases=["passwordgenerate", "generatepassword", "genpassword"])
    ...
    @Ghast.command(name="ccgen", description="Generate a fake Credit card.", usage="ccgen", aliases=["creditcardgenerate", "creditcardgen", "generatecc", "ccgenerate", "gencreditcard", "generatecreditcard"])
    async def ccgen(ctx):
        ...
        if choice == 4:
            type = "Visa"
            typeimg = "https://Ghast.cool/assets/visa.png"
        elif choice == 5:
            type = "Mastercard"
            typeimg = "https://Ghast.cool/assets/mastercard.png"
        elif choice == 6:
            type = "Discover"
            typeimg = "https://Ghast.cool/assets/discover.png"
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="cembed", description="Create a custom embedded message.", usage='cembed [title] [description] [colour]', aliases=["customembed"])
    ...
    @Ghast.command(name="embed", description="Create an embedded message.", usage="embed [title]")
    ...
    @Ghast.command(name="leet", description="Turn your text into 1337 text.", usage="leet [text]", aliases=["1337", "leetspeak"])
    async def leet(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://Ghast.cool/api/fun/leet?text={text}").text)

    @Ghast.command(name="zalgo", description="Unleash the zalgo into your message.", usage="zalgo [text]")
    async def zalgo(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://Ghast.cool/api/fun/zalgo?text={text}").text)

    @Ghast.command(name="upsidedown", description="Flip your text upsidedown.", usage="upsidedown [text]")
    async def upsidedown(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://Ghast.cool/api/fun/upsidedown?text={text}").text)

    @Ghast.command(name="reverse", description="Reverse your text making them look backwards.", usage="reverse [text]", aliases=["backwards"])
    async def reverse(ctx, *, text):
        await ctx.send(''.join(list(reversed(text))))

    @Ghast.command(name="ascii", description="Send your message in ascii.", usage="ascii [text]")
    ...
    @Ghast.command(name="privatemsg", description="Send an encrypted message.", usage="privatemsg [message]", aliases=["b64encode", "privatemessage"])
    ...
    @Ghast.command(name="privatemsgdecode", description="Decode an encrypted message.", usage="privatemsgdecode [message]", aliases=["b64decode", "privatemessagedecode"])
    ...
    @Ghast.command(name="encodebinary", description="Encode a message in binary.", usage="encodebinary [message]", aliases=["binaryencode", "binary"])
    async def encodebinary(ctx, *, message):
        translation = ""

    @Ghast.command(name="decodebinary", description="Decode a message in binary.", usage="decodebinary [message]", aliases=["binarydecode", "unbinary"])
    async def decodebinary(ctx, *, message):
        translation = ""

    @Ghast.command(name="encodemorse", description="Encode a message in morsecode", usage="encodemorse [message]", aliases=["morseencode", "morse"])
    async def encodemorse(ctx, *, message):
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://Ghast.cool/api/fun/encodemorse?text={text}").text)

    @Ghast.command(name="decodemorse", description="Decode a message in morsecode", usage="decodemorse [message]", aliases=["morsedecode", "unmorse"])
    async def decodemorse(ctx, *, message):
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://Ghast.cool/api/fun/decodemorse?text={text}").text)                

    @Ghast.command(name="secret", description="Send all your messages in a secret block.", usage="secret [message]")
    async def secret(ctx, *, message):
        await ctx.send('||' + message + '||')

    @Ghast.command(name="secretletters", description="Put all lettes from your message into separate secret blocks", usage="secretletters [message]")
    ...
    @Ghast.command(name="bold", description="Send all your messages in bold.", usage="bold [message]")
    async def bold(ctx, *, message):
        await ctx.send('**' + message + '**')

    @Ghast.command(name="italic", description="Send all your messages in italics.", usage="italic [message]")
    async def italic(ctx, *, message):
        await ctx.send('*' + message + '*')

    @Ghast.command(name="cpp", description="Send all your messages in a C++ code block.", usage="cpp [message]")
    async def cpp(ctx, *, message):
        await ctx.send(f"""```cpp\n{message}```""")

    @Ghast.command(name="cs", description="Send all your messages in a C Sharp code block.", usage="cs [message]")
    async def cs(ctx, *, message):
        await ctx.send(f"""```cs\n{message}```""")

    @Ghast.command(name="java", description="Send all your messages in a Java code block.", usage="java [message]")
    async def java(ctx, *, message):
        await ctx.send(f"""```java\n{message}```""")

    @Ghast.command(name="python", description="Send all your messages in a Python code block.", usage="python [message]")
    async def python(ctx, *, message):
        await ctx.send(f"""```py\n{message}```""")

    @Ghast.command(name="js", description="Send all your messages in a JavaScript code block.", usage="js [message]")
    async def js(ctx, *, message):
        await ctx.send(f"""```js\n{message}```""")
    @Ghast.command(name="lua", description="Send all your messages in a Lua code block.", usage="lua [message]")
    async def lua(ctx, *, message):
        await ctx.send(f"""```lua\n{message}```""")

    @Ghast.command(name="php", description="Send all your messages in a PHP code block.", usage="php [message]")
    async def php(ctx, *, message):
        await ctx.send(f"""```php\n{message}```""")

    @Ghast.command(name="html", description="Send all your messages in a HTML code block.", usage="html [message]")
    async def html(ctx, *, message):
        await ctx.send(f"""```html\n{message}```""")

    @Ghast.command(name="css", description="Send all your messages in a CSS code block.", usage="css [message]")
    async def css(ctx, *, message):
        await ctx.send(f"""```css\n{message}```""")

    @Ghast.command(name="yaml", description="Send all your messages in a YAML code block.", usage="yaml [message]")
    async def yaml(ctx, *, message):
        await ctx.send(f"""```yaml\n{message}```""")

    @Ghast.command(name="json", description="Send all your messages in a JSON code block.", usage="json [message]")
    async def _json(ctx, *, message):
        await ctx.send(f"""```json\n{message}```""")

    @Ghast.command(name="aesthetic", description="Send your text s p a c e d out.", usage="aesthetic [text]")
    ...
    @Ghast.command(name="animate", description="Animate your text.", usage="animate [text]")
    ...
    @Ghast.command(name="chatbypass", description="Bypass chat language restrictions.", usage="chatbypass [text]", aliases=["bypasschat"])
    ...
    @Ghast.command(name="regional", description="Replace all letters with emoji.", usage="regional [text]")
    ...
    @Ghast.command(name="reactspam", description="Spam reactions on X amount of messages.", usage="reactspam [emoji] [messages]", aliases=["spamreactions", "spamreact"])
    async def reactspam(ctx, emoji, messages: int):
        if __riskmode__:
            #channel = Ghast.get_channel(ctx.channel.id)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @Ghast.command(name="uppercase", description="Send your message in uppercase.", usage="uppercase [msg]")
    async def uppercase(ctx, *, msg):
        string = msg.upper()
        await ctx.send(string)

    @Ghast.command(name="lowercase", description="Send your message in lowercase.", usage="lowercase [msg]")
    async def lowercase(ctx, *, msg):
        string = msg.lower()
        await ctx.send(string)

    @Ghast.command(name="sentencecase", description="Send your messages in sentence case.", usage="sentencecase [msg]")
    ...
    @Ghast.command(name="banlist", description="See the server's ban list.", usage="banlist")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="ban", description="Ban the mentioned user.", usage="ban [@user]")
    ...
    @Ghast.command(name="unban", description="Unban the mentioned id.", usage="unban [id]")
    async def unban(ctx, *, id: int):
        if ctx.author.guild_permissions.ban_members:
            user = await Ghast.fetch_user(id)
    ...
    @Ghast.command(name="kick", description="Kick the mentioned user.", usage="kick [@user]")
    ...
    @Ghast.command(name="mute", description="Mute the menitioned user.", usage="mute [@user]")
    ...
    @Ghast.command(name="unmute", description="Unmute the mentioned user.", usage="unmute [@user]")
    ...
    @Ghast.command(name="newrole", description="Create a new role.", usage="newrole [name]", aliases=["createrole"])
    ...
    @Ghast.command(name="delrole", description="Delete the mentioned role.", usage="delrole [@role]", aliases=["deleterole"])
    ...
    @Ghast.command(name="purge", description="Purge X amount of messages.", usage="purge [amount]")
    ...
    @Ghast.command(name="purgeself", description="Purge your messages.", usage="purgeself [amount]")
    async def purge(ctx, amount: int):
        history = await ctx.channel.history(limit=amount).flatten()
        deletedamount = 0
        for message in history:
    ...
    @Ghast.command(name="lock", description="Lock the command channel.", usage="lock")
    async def lock(ctx):
        if ctx.author.guild_permissions.manage_channels:
    ...
    @Ghast.command(name="unlock", description="Unlock the command channel.", usage="unlock")
    ...
    @Ghast.command(name="lockdown", description="Lock the entire server.", usage="lockdown")
    ...
    @Ghast.command(name="unlockdown", description="Unlock the entire server.", usage="lockdown")
    ...
    @Ghast.command(name="tokeninfo", description="Information about a token.", usage="tokeninfo [token]")
...
# {__embedfooter__}```{avatar}""")  
    ...
    @Ghast.command(name="userinfo", description="Information about the mentioned user.", usage="userinfo [@user]", aliases=["userlookup", "lookupuser"])
...
# {__embedfooter__}```{avatarUrl(user.id, user.avatar)}""")
    ...
    @Ghast.command(name="serverinfo", description="Information about the command server.", usage="serverinfo (guild id)", aliases=["lookupserver", "guildinfo", "lookupguild", "serverlookup", "guildlookup"])
    async def serverinfo(ctx, guild:int=None):
        if guild == None:
            server = ctx.message.guild
        else:
            server = await Ghast.fetch_guild(int(guild))
        if __embedmode__:
...
# {__embedfooter__}```{str(server.icon)}""")                        
    ...
    @Ghast.command(name="avatar", description="Get the mentioned user's avatar.", usage="avatar [@user]", aliases=["pfp", "profilepicture"])
    ...
    @Ghast.command(name="servericon", description="Get the server's icon.", usage="servericon", aliases=["guildicon"])
    ...
    @Ghast.command(name="afkmode", description="Toggle afk mode.", usage="afkmode")
    ...
    @Ghast.command(name="settings", description="The bot's settings.", usage="settings")
    async def settings(ctx):
        totalguilds = len(Ghast.guilds)
        totalcommands = len(Ghast.commands) + len(ccmd)

        uptime = int(round(time.time() - botStartTime))
        uptimeText = str(timedelta(seconds=uptime))

        delta_uptime = datetime.now() - Ghast.launch_time
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Prefix", value=f"```{Ghast.command_prefix}```")
            ...
            embed.add_field(name="Credits", value="""Thanks to all the below people for helping with some of Ghast's commands.
```
Imf44 : Fox command.
Qoft : Proxy related commands.
Benny : The old version of Ghost.cool
```
            """, inline=False)
            embed.set_thumbnail(url=__embedimage__)
            embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    '''@Ghast.command(name="snipers", description="All snipers.", usage="snipers")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)'''
    ...
    @Ghast.command(name="playing", description="Set a playing status.", usage="playing [text]")
    async def playing(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                
        await Ghast.change_presence(activity=discord.Game(text), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="streaming", description="Set a streaming status.", usage="streaming [text]")
    async def streaming(ctx, url, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await Ghast.change_presence(activity=discord.Activity(type=1, name=f"{text}", url=f"{url}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="listening", description="Set a listening to status.", usage="listening [text]")
    async def listening(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await Ghast.change_presence(activity=discord.Activity(type=2, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="watching", description="Set a watching status.", usage="watching [text]")
    async def watching(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await Ghast.change_presence(activity=discord.Activity(type=3, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @Ghast.command(name="clearstatus", description="Clear your status.", usage="clearstatus")
    async def clearstatus(ctx):
        await Ghast.change_presence(activity=discord.Activity(type=-1), status=discord.Status.try_value(status))
        try:
    ...
    @Ghast.command(name="nickname", description="Set your nickname to anything.", usage="nickname [text]")
    ...
    @Ghast.command(name="clearnickname", description="Clear your nickname.", usage="clearnickname")
    ...
    # GhastDiscum = discum.Client(token=__token__, log=False, user_agent=get_random_user_agent())

    # @GhastDiscum.gateway.command
